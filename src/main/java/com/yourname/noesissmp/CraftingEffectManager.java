...
public class CraftingEffectManager {
     ...
     * Gentle starlight and soft particles drifting gracefully within 1.5 blocks around the table.
     */
    public static void playAmbientEffect(Location blockLoc) {
        ...
        // 1. Subtle, gentle floating starlight particles around top of table (radius ~1.2m)
        for (int i = 0; i < 3; i++) {
        ...
        if (random.nextDouble() < 0.4) {
            coreLoc.getWorld().spawnParticle(
                    Particle.ENCHANT,
                    coreLoc.clone().add((random.nextDouble() - 0.5) * 0.8, 0.2 + random.nextDouble() * 0.4, (random.nextDouble() - 0.5) * 0.8),
                    2, 0.1, 0.1, 0.1, 0.2
            );
        }
     ...
     * Creates a crisp, beautiful ring, vertical light beam, and satisfying chime without screen clutter.
     */
